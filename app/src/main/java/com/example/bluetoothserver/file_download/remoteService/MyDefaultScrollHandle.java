package com.example.bluetoothserver.file_download.remoteService;/*
package com.example.docconnect.ui.file_download.remoteService;
...
public class MyDefaultScrollHandle extends RelativeLayout implements ScrollHandle {
    ...
    public MyDefaultScrollHandle(Context context) {
        this(context, false);
    }
    public MyDefaultScrollHandle(Context context, onXYListener onXYListener) {
        this(context, false);
        this.onXYListener = onXYListener;
    }

    public MyDefaultScrollHandle(Context context, boolean inverted) {
        super(context);
...
//        onXYListener.setPosition((pdfView.isSwipeVertical() ? pdfView.getHeight() : pdfView.getWidth()) * position);
    }
    ...
    public void setPosition(float pos) {
        ...
        if (pdfView.isSwipeVertical()) {
            setY(pos);
//            onXYListener.setY(pos);
        } else {
            setX(pos);
//            onXYListener.setX(pos);
        }
...
//                    onXYListener.setPosition(event.getRawY() - currentPos + relativeHandlerMiddle);
            pdfView.setPositionOffset(relativeHandlerMiddle / (float) getHeight(), false);
//                    onXYListener.setPositionOffset(relativeHandlerMiddle / (float) getHeight());
        } else {
            setPosition(event.getRawX() - currentPos + relativeHandlerMiddle);
//                    onXYListener.setPosition(event.getRawX() - currentPos + relativeHandlerMiddle);
            pdfView.setPositionOffset(relativeHandlerMiddle / (float) getWidth(), false);
//                    onXYListener.setPositionOffset(relativeHandlerMiddle / (float) getWidth());
        }
    }
    ...
    interface onXYListener {
        void setX(float x);
        void setY(float y);

        void setPosition(float y);
        void setPositionOffset(float y);
    }
}
*/
